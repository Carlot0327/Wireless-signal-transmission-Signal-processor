...
{
    WS2812B_SetPixel(num, (rgb >> 16) & 0xFF,(rgb >> 8) & 0xFF, (rgb >> 0) & 0xFF);  
...
void WS2812B_Light(uint16_t num, uint32_t rgb)
{
		WS2812B_SetPixelRGB(num,rgb);
		WS2812B_SetPixelRGB((num + 1) % 3,NONE);
		WS2812B_SetPixelRGB((num + 2) % 3,NONE);
		WS2812B_Update();
		HAL_Delay(5);
}
